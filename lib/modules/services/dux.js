...
import { parseRealmData } from '../realmdata/dux'
...
export function cache(services) {
  const data = _.map(services, (service) => ({
    id: service.id,
    organizationId: service.organizationId,
    name: service.name,
    image: service.image,
    isRealm: service.isRealm,
    ...service.app
  }))
...
export function save(params) {
  return (dispatch, getState) => {
    const serviceCache = getState().services[params.id] || {}
    ...
    return fetch(url, { timeout: 15000 })
      .then((response) => {
        ...
        if (response.status !== 200) {
          ...
          if (id === 'elysium_website') {
            dispatch(parseRealmData(response))
          }
        }
        dispatch(save({ status, lastUpdated, id }))
        // continue polling
